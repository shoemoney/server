...
not_locked:
		...
		if (!extern_cols || DBUG_IF("purge_no_blob_freeze")) {
			...
			being locked by mtr is enough. The keyword sends a
			version that does have externally stored columns
			here as well, which is how the dereference below
			...
			/* The keyword forces this branch, which is
			otherwise reached only when trx commits between
			the walk starting and the freeze. */
			bool purgeable =
				freeze.view().changes_visible(trx->id);
			DBUG_EXECUTE_IF("row_vers_impl_x_locked_purgeable",
					purgeable = true;);
			if (purgeable) {
				goto not_locked;
			}
