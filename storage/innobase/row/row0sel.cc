             ...
             !view.sees(page_trx_id))
    {
      ...
      check_latest_version:
        ...
        to ask about, which is what both tests below need. */
        {
          ...
          {
            /* The keyword sends a record that does have externally stored
            columns here as well, which is how the fetch below behaved
      ...
      check_old_vers:
        ...
        for (;;)
        {
          ...
          if (&view != &prebuilt->trx->read_view)
          {
            ...
            {
              /* The keyword sends a version that does have externally
              stored columns here as well, which is how the fetch below
              behaved before it was protected. */
              if (extern_cols)
            ...
            else
            {
              ...
              The keyword forces this branch, which is otherwise reached
              only when the view advances between the walk starting and
              the freeze. */
              bool purgeable= freeze.view().changes_visible(version_trx_id);
              DBUG_EXECUTE_IF("row_check_index_purgeable", purgeable= true;);
              if (purgeable)
              {
                mem_heap_free(vers_heap);
                goto did_not_find;
