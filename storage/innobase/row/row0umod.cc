...
nochange_index:
		...
		if (!rec_get_deleted_flag(prev_version, comp)) {
			...
			if (!extern_cols
				...
				versions being locked by mtr is enough. The
				keyword sends a version that does have
				externally stored columns here as well, which
				is how the dereference below behaved before it
				was protected. */
				if (extern_cols) {
					DEBUG_SYNC_C(
				...
				otherwise reached only when the view advances
				between the walk starting and the freeze. */
				bool purgeable = freeze.view().changes_visible(
					version_trx_id);
				DBUG_EXECUTE_IF(
